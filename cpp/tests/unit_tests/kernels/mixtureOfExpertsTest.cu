...
protected:
    ...
    constexpr static bool W4A8_AWQ = std::is_same_v<GemmDataType, SafeFP8> && std::is_same_v<WeightType, cutlass::uint4b_t>;
    constexpr static bool INT_QUANT = !std::is_same_v<GemmDataType, WeightType> && std::is_integral_v<WeightType> && !W4A8_AWQ;
        ...
        MOEParallelismConfig parallelism_config)
    {
        ...
        {
            cutlass_kernels::QuantType quant_type
                = cutlass_kernels::QuantType::W4_AFP8;
        ...
        scale_type* scales, uint8_t* outputs)
    {
        ...
        if constexpr (INT_QUANT)
        {
            ...
            for (int expert = 0; expert < num_experts; ++expert)
            {
                ...
                {
                    WeightRawType const* current_weight_group = current_weight + ii * mGroupSize;
                    int8_t* current_quantized_weight_group = current_quantized_weight + ii * mGroupSize * bits_per_weigtht_element / 8;
                ...
                // WAR: For a diagonal matrix in which each column has only one nonzero element, the scale value is calculated based on it being quantized to 8. 
                // However, after quantization, the nonzero value is clipped to 7. Adjust the scale value to offset the error.
                for (int ii = 0; ii < input_mat_size / mGroupSize; ++ii)
                {
    ...
    void runMoEPermute(MOEParallelismConfig parallelism_config, bool enable_alltoall = false)
    {
        ...
        else if constexpr (W4A8_AWQ)
        {
            ...
            check_cuda_error(cudaMemcpy(input_scale1, h_input_scale1.data(), mNumExperts * mHiddenSize * mGatedMultiplier * sizeof(scale_type), cudaMemcpyHostToDevice));
            check_cuda_error(cudaMemcpy(input_scale2, h_input_scale2.data(), mNumExperts * mInterSize * sizeof(scale_type), cudaMemcpyHostToDevice));
            ...
            ASSERT_TRUE(scale1_ptr && scale2_ptr);
            quant_params = QuantParams::GroupWise(mGroupSize, scale1_ptr, scale2_ptr, input_scale1, input_scale2, nullptr, nullptr, alpha1_ptrs, alpha2_ptrs);
        }
        else if (FP8)
        {
