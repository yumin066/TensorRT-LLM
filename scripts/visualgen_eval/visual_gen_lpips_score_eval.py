...
def _decode_video_to_lpips_batch(
    ...
    # Optionally restrict scoring to a half-open frame window [frame_start, frame_stop).
    # The slice operates on the originally decoded frames, so no re-encoding is
    # introduced and per-frame pixels are byte-identical to a full-video decode.
