...
class BasicAVTransformerBlock(nn.Module):
    ...
    def _text_cross_attention_input_scale(
        self, attention: LTX2Attention, args: TransformerArgs
    ) -> Optional[torch.Tensor]:
        """Return the optional pre-quantization scale for text cross-attention."""
        return get_nvfp4_input_scale(attention.to_q)
        ...
        if run_vx:
            ...
            if not skip_v_self:
                ...
                vx, attn2_q_input = apply_fused_gate_resid_rmsnorm(
                    ...
                    self.norm_eps,
                    self._fuse_adaln,
                    fp4_input_scale=self._text_cross_attention_input_scale(self.attn2, video),
                )
            else:
        ...
        if run_ax:
            ...
            if not skip_a_self:
                ...
                ax, audio_attn2_q_input = apply_fused_gate_resid_rmsnorm(
                    ...
                    self.norm_eps,
                    self._fuse_adaln,
                    fp4_input_scale=self._text_cross_attention_input_scale(self.audio_attn2, audio),
                )
            else:
