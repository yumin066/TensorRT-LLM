...
class AudioEncoderConfigurator:
    ...
    def from_config(cls, config: dict) -> AudioEncoder:
        ...
        # Upstream selects the attention block by ``attn_type``; this port only
        # implements the "vanilla" block. Reject anything else rather than
        # silently building vanilla attention where upstream would have built
        # ``Identity`` (attn_type="none") or raised (attn_type="linear").
