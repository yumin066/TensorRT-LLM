...
# modality dataclass and transformer args) are scoped to the retake package;
# unchanged architecture helpers are shared with the native LTX-2 package.
...
from ..ltx2.ltx2_core.perturbations import BatchedPerturbationConfig, PerturbationType
...
from ..ltx2.text_cache import TextCache
...
class LTX2Attention(Attention):
    ...
    def __init__(
        ...
        # Validate Ulysses head divisibility.
...
class BasicAVTransformerBlock(nn.Module):
    ...
    def _init_audio_modules(self, cfg, rope_type, eps, model_config, idx):
        ...
        num_adaln_slots = 9 if cfg.cross_attention_adaln else 6
        self.audio_scale_shift_table = nn.Parameter(torch.empty(num_adaln_slots, cfg.dim))
        if cfg.cross_attention_adaln:
    ...
    def forward(
        ...
        if run_vx:
            ...
            if use_cross_adaln:
                text_v_attn_raw = self._run_text_cross_adaln(
                    attn2_q_input,
                    video,
                    self.attn2,
        ...
        if run_ax:
            ...
            if use_audio_cross_adaln:
                text_a_attn_raw = self._run_text_cross_adaln(
                    audio_attn2_q_input,
                    audio,
                    self.audio_attn2,
...
class LTXModel(BaseDiffusionModel):
    ...
    # from TRT-LLM names after QKV fusion and FF remapping. Remap exclusion
    # patterns so non-quantized layers remain excluded.
