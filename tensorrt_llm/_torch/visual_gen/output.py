...
class PipelineOutput:
    ...
    # conditioning, denoise_total, denoise_per_step, vae_decode, postprocess).
...
class RetakeStageTimer:
    ...
    ``denoise_total``, ``vae_decode``, ``postprocess``, and a ``denoise_per_step``
    ...
    def fill(self, output: "PipelineOutput") -> "PipelineOutput":
        ...
        output.stage_timings = {
            ...
            "vae_decode": _dur("decode", "postprocess"),
            "postprocess": _dur("postprocess", "end"),
        }
        # Public three-phase view (back-compatible): pre = source_read + encode +
        # conditioning, denoise = the loop, post = decode + postprocess.
