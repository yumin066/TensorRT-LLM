...
def _generate_ltx2_retake_native_bf16_video(llm_venv, tmp_path, output_path):
    ...
    base_config = os.path.join(
        REPO_ROOT, "examples", "visual_gen", "configs", "ltx2-retake-1gpu.yaml"
    )
    with open(base_config, encoding="utf-8") as config_file:
        config_data = yaml.safe_load(config_file)
    config_data["pipeline_config"].update(
        {
            "retake_prompt_conditioning_path": str(prompt_conditioning),
...
@pytest.fixture(scope="session")
def ltx2_retake_bf16_video_path(_ltx2_retake_deps, llm_venv, tmp_path_factory):
    output_path = _visual_gen_output_path(llm_venv, "ltx2_retake_bf16")
    if os.path.isfile(output_path):
        return output_path
    work_dir = tmp_path_factory.mktemp("ltx2_retake")
    return _generate_ltx2_retake_native_bf16_video(llm_venv, work_dir, output_path)
