...
"""CPU tests for the LTX-2 native audio VAE encoder and its retake call path.

The retake window conditions the video denoise on a *frozen* audio latent, and
the whole joint denoise drifts from the reference if that latent is off — an
earlier ~5e-5 audio drift was enough to break video parity from step 5 onward.
Until now the latent came from the upstream `ltx_pipelines` helper; these tests
cover the native replacement.

What is and is not proven here, so neither is mistaken for the other:

  * the pure pieces are EXECUTED: causal downsample padding, latent-length
    conform, the audio-frame format conversion, the config mapping, and an
    end-to-end forward of a tiny randomly-initialized encoder.
  * the dependency removal is checked STRUCTURALLY with `ast`
    (`*_upstream_audio_helper*`), because the native retake method it guards
    needs a checkpoint, weights and a GPU to execute.
    `test_the_upstream_audio_helper_check_can_fail` shows that check
    discriminates.

Numeric parity of the encoder against the reference implementation is NOT
provable here — it needs the real checkpoint — and is established separately by
the parity harness on GPU.

Everything below runs on CPU without a checkpoint.
"""
...
# The audio VAE shape contract the retake path depends on.
LATENT_DOWNSAMPLE_FACTOR = 4


# --------------------------------------------------------------------------------------
# Causal downsampling geometry
# --------------------------------------------------------------------------------------
...
def test_downsample_pads_asymmetrically_per_causality_axis(axis, expected_pad):
    ...
    the same output *shape*, so only the values reveal the error. `expected_pad`
    is the reference implementation's tuple, transcribed here as the pin.
    """
    torch.manual_seed(0)
...
def test_configurator_rejects_an_unsupported_attention_type():
    """This port only implements vanilla attention.

    Upstream builds `Identity` for attn_type="none"; silently building vanilla
    attention instead would change the encode with no error anywhere.
    """
    with pytest.raises(ValueError, match="attn_type"):
        AudioEncoderConfigurator.from_config(_encoder_config(attn_type="none"))


def test_encoder_halves_both_axes_per_level_and_emits_z_channels():
    """End-to-end forward of a tiny encoder: the shape contract the retake path assumes.

    Each downsampling level halves time *and* mel bins, and `double_z=True` means
    the final convolution emits `2 * z_channels` — but only the mean half survives
    `_normalize_latents`, so the returned latent must carry `z_channels`, not
    twice that. Getting the chunk wrong would silently feed the log-variance half
...
# --------------------------------------------------------------------------------------
# AC-7 — the native retake path must not use the upstream audio helper.
#
# The oracle path (`retake_use_upstream_stage=True`) is deliberately preserved as
# a named reference, so it is the one place the upstream helper may still appear.
# --------------------------------------------------------------------------------------

# Methods that belong to the preserved upstream oracle, not the native path.
_ORACLE_ONLY_FUNCTIONS = frozenset({"_import_upstream_retake_symbols", "_run_native_retake"})


def _upstream_audio_helper_uses(tree, allowed_functions=frozenset()):
    """Every mention of `audio_latent_from_file` outside *allowed_functions*.

    Keyed by enclosing function name rather than line number so the check keeps
    pointing at the same code as the file shifts. Imports, attribute accesses and
    bare names all count: the dependency is removed only if none remain.
    """
    offending = []
    for func in ast.walk(tree):
        ...
        for node in ast.walk(func):
            mentioned = (
                (isinstance(node, ast.Name) and node.id == "audio_latent_from_file")
                or (isinstance(node, ast.Attribute) and node.attr == "audio_latent_from_file")
                or (isinstance(node, ast.alias) and node.name == "audio_latent_from_file")
            )
            if mentioned:
                offending.append((func.name, node.lineno))
    return offending


def test_native_retake_does_not_use_the_upstream_audio_helper():
    """AC-7's delivery requirement: the native path drives the audio encode itself."""
    import inspect
    ...
    offending = _upstream_audio_helper_uses(tree, allowed_functions=_ORACLE_ONLY_FUNCTIONS)
    assert not offending, (
        "the native retake path still reaches for the upstream "
        f"`audio_latent_from_file`: {offending}"
    )


def test_the_upstream_audio_helper_check_can_fail():
    """Prove the structural check discriminates, on each shape a regression takes."""
    for bad, why in (
        (
            "def _run_native_pre_post_retake(self):\n"
            "    from ltx_pipelines.utils.helpers import audio_latent_from_file\n",
            "direct import",
        ),
        (
            "def _run_native_pre_post_retake(self):\n"
            "    return audio_latent_from_file(enc, path, shape, dev, dt)\n",
            "bare call",
        ),
        (
            "def _run_native_pre_post_retake(self):\n    return up.audio_latent_from_file(enc)\n",
            "attribute call",
        ),
    ):
        found = _upstream_audio_helper_uses(
            ast.parse(bad), allowed_functions=_ORACLE_ONLY_FUNCTIONS
        )
        assert found, f"checker missed the {why} regression shape"


def test_the_oracle_path_is_still_allowed_to_use_the_upstream_helper():
    """The exclusion list must name real functions, or it silences nothing.

    If `_run_native_retake` were renamed, the entry would go stale and the check
    would quietly widen; asserting the oracle really does still use the helper
    ...
    tree = ast.parse(inspect.getsource(pipeline_ltx2_retake))
    oracle_uses = [
        name for name, _line in _upstream_audio_helper_uses(tree) if name in _ORACLE_ONLY_FUNCTIONS
    ]
    assert set(oracle_uses) == set(_ORACLE_ONLY_FUNCTIONS), (
        "the oracle exclusion list no longer matches the functions that use the "
        f"upstream helper: {sorted(set(oracle_uses))}"
    )


def test_native_pre_post_calls_the_native_audio_encode():
    """The native path must actually route through the native encode method."""
    import inspect

    from tensorrt_llm._torch.visual_gen.models.ltx2.pipeline_ltx2_retake import LTX2RetakePipeline

    source = inspect.getsource(LTX2RetakePipeline._run_native_pre_post_retake)
    calls = [
        node
        for node in ast.walk(ast.parse(source.lstrip()))
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Attribute)
        and node.func.attr == "_encode_source_audio_latent"
    ]
    assert len(calls) == 1, "expected exactly one native audio encode in the native retake path"


# --------------------------------------------------------------------------------------
# The native audio encode composition (executed against stubs)
# --------------------------------------------------------------------------------------
